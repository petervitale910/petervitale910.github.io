...
First we import data which was requested from [@MSI_SF_Bay_Institute]. This data encapsulates individual otter trawls which were taken in the Bay between 1992 and 2017. The net is kept in the water for 5 minutes and then retrieved. A majority of the trawls are taken in the South Bay, however there are consistent year over year trawls in the other regions (Central bay, San Pablo bay, and Suisun bay).
...
p1 <- ggplot(catch_plot, aes(x = date, y = number,
                       color = species_group)) +
  geom_point(alpha = .3) +
  ggthemes::theme_few() +
...
catch_plot <- catch_plot %>% 
  dplyr::filter(expr1 > 1980 & expr1 < 1990)

p2 <- ggplot(catch_plot, aes(x = date, y = number,
                       color = species_group)) +
  geom_point(alpha = .3) +
  ggthemes::theme_few() +
  labs(x = "Date",
